      ...
      <section className="relative overflow-hidden rounded-[32px] bg-slate-950 px-6 py-8 text-white shadow-[0_36px_90px_-44px_rgba(15,23,42,0.82)] sm:px-8 lg:px-10">
        ...
        <div className="relative">
          ...
          <h2 className="mt-4 max-w-3xl text-3xl font-semibold tracking-tight sm:text-4xl xl:text-[2.9rem]">
            Build platform-ready ads in seconds.
          </h2>

          <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">
            An AI pipeline plans your strategy, generates structured copy, validates output, and retries automatically.
          </p>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {PRESET_BRIEFS.map((preset) => (
              <button
                ...
                disabled={isRunning}
                className="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/18 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {preset.label}
              </button>
            ))}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(400px,0.92fr)] 2xl:grid-cols-[minmax(0,1.05fr)_minmax(500px,0.95fr)]">
          ...
          <div className="border-b border-slate-200/80 px-6 py-6 sm:px-7">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50/90 p-4 shadow-inner shadow-white/70 lg:w-full lg:max-w-[250px]">
                ...
                <p className="mt-3 text-sm text-slate-600">{requiredFieldsComplete}/3 required fields complete.</p>
              </div>
            </div>
          ...
          <div className="space-y-6 p-6 sm:p-7">
            ...
            <div className="rounded-[26px] border border-slate-200 bg-white/85 p-5 shadow-sm shadow-slate-900/5">
              ...
              <input
                ...
                disabled={isRunning}
                className="mt-4 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-slate-950 focus:ring-4 focus:ring-slate-200/70 disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <div className="rounded-[26px] border border-slate-200 bg-white/85 p-5 shadow-sm shadow-slate-900/5">
              ...
              <textarea
                ...
                disabled={isRunning}
                rows={4}
                className="mt-4 w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-slate-950 focus:ring-4 focus:ring-slate-200/70 disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <div className="rounded-[26px] border border-slate-200 bg-white/85 p-5 shadow-sm shadow-slate-900/5">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-slate-500">Pick a common channel quickly or type a custom placement below.</p>
              </div>
                ...
                {PLATFORM_OPTIONS.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isRunning}
                      className={`rounded-[22px] border px-4 py-4 text-left transition disabled:cursor-not-allowed disabled:opacity-60 ${
                        active
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                ...
                disabled={isRunning}
                className="rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Load Example
              </button>
              <button
        ...
        <section className="space-y-5 xl:sticky xl:top-24 xl:self-start">
          <div className="rounded-[30px] border border-white/70 bg-white/82 p-5 shadow-[0_28px_70px_-42px_rgba(15,23,42,0.28)] backdrop-blur sm:p-6">
            ...
            {/* Planning state — shows while planning, persists as plan card after */}
            {plan && (
              <div className="mt-5 rounded-[22px] border border-indigo-200 bg-indigo-50/80 p-4">
            ...
            {/* Planning skeleton — shows when planning but no plan yet */}
            {pipelineStage === "planning" && !plan && (
              <div className="mt-5 rounded-[22px] border border-indigo-200 bg-indigo-50/50 p-4">
              ...
              <div className="rounded-[27px] bg-white p-5">
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className={`rounded-2xl px-4 py-3 text-right ${scoreTone.track}`}>
                    ...
                    {ad && (
                      <div className={`rounded-2xl px-4 py-3 text-right ${confidenceTone.track}`}>
                        <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Confidence</p>
                        <p className="mt-1 text-2xl font-semibold text-slate-950">{confidenceScore}</p>
                        <p className={`mt-1 inline-flex rounded-full px-2 py-1 text-[11px] font-semibold ${confidenceTone.badge}`}>
                  ...
                  {isRunning && pipelineStage !== "planning" ? (
                    ...
                    <div className="text-center">
                      ...
                      <p className="mt-3 text-sm leading-6 text-slate-500">
                        Fill out the brief, then generate to see a polished preview with scores and refinements.
                      </p>
                    </div>
                  ) : (
                    <div>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              <button
                ...
                onClick={generateAd}
                disabled={!ad || isRunning}
                className="rounded-full bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-indigo-300"
              >
                Refine Output
              </button>
              <button
